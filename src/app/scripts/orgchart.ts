...
export abstract class OrgChartUtilityMethods {
    ...
    public static readFile(event: ProgressEvent): void {
        ...
        if (this.fileType === 'csv') {
            ...
            let xmlDom: XMLDocument = parser.parseFromString(<string>(event.target as FileReader).result, 'text/xml');
    ...
    public static addChild(sourceId: string): void {
        ...
        if (parentNode.shape && parentNode.shape.type === 'Image') {
            node.shape = { type: 'Image', source: './assets/syncfusion/dbstyle/orgchart_images/blank-male.jpg', align: 'XMinYMin', scale: 'Meet' };
        } else {
